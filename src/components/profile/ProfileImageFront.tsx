...
const ProfileImageFront = (props: Props) => {
  ...
  return (
    ...
    >
      ...
      <StopPropagationDiv>
        <Link
          className="pl-6 pt-6 pb-2 pr-2 absolute -bottom-[2px] right-0"
          href={`/posts/${postId}`}
          scroll={false}
        >
          <Expand className="visible sm:invisible group-hover:visible size-[28px] text-slate-200 hover:scale-110 transition duration-300 ease-in-out" />
        </Link>
      </StopPropagationDiv>
    </MotionDiv>
  );
